...
extern "C" {
  ...
  STDAPI           BuildIoRingReadFile(HIORING ioRing, IORING_HANDLE_REF fileRef, IORING_BUFFER_REF dataRef, UINT32 numberOfBytesToRead, UINT64 fileOffset, UINT_PTR userData, IORING_SQE_FLAGS sqeFlags);
  STDAPI           BuildIoRingRegisterFileHandles(HIORING ioRing, UINT32 count, const HANDLE handles[], UINT_PTR userData);
  STDAPI           BuildIoRingRegisterBuffers(HIORING ioRing, UINT32 count, const IORING_BUFFER_INFO buffers[], UINT_PTR userData);
