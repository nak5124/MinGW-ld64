...
extern "C" {
  ...
  WINBASEAPI VOID    WINAPI ExitProcess(UINT uExitCode) DECLSPEC_NORETURN;
  ...
  WINBASEAPI VOID    WINAPI ExitThread(DWORD dwExitCode) DECLSPEC_NORETURN;
...
# define GetStartupInfo GetStartupInfoW
  ...
  FORCEINLINE
  HANDLE GetCurrentThreadToken(VOID)
  {
    return (HANDLE)(LONG_PTR)(-5);
  }

  FORCEINLINE
  HANDLE GetCurrentThreadEffectiveToken(VOID)
  {
...
#define PROCESS_AFFINITY_ENABLE_AUTO_UPDATE 0x00000001UL
  ...
  WINBASEAPI WINBOOL WINAPI GetSystemTimes(PFILETIME lpIdleTime, PFILETIME lpKernelTime, PFILETIME lpUserTime);
